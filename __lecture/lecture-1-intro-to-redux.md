...
const FridgeContents = () => {
  const fridgeItems = useSelector((state) => {
    return state.fridge; //THIS IS A FUNCTION
  });
...
const App = () => {
  ...
  const movie = useSelector((state) => {
    const bfGenre = state.boyfriendFavouriteGenre;
    return state.myFavouriteMovies[bfGenre];
  });
  ...
  return (
    <ul>
      {todos.map(todo => (
        <li key={todo.id}>
          {todo.value}
          <button onClick={() => dispatch({type: markTodoAsCompleted(todo.id)})}>
        </li>
      ))}
    </ul>
  );
